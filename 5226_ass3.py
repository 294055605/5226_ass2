...
class Agent:
    def __init__(self, env: Environment, reward=0, learning_rate=0.005, discount_factor=0.9, exploration_rate=1,
    ...
    def choose_action(self, env):
        ...
        """
        state = self.prepare_state((location_to_onehot(env.item_location, env.height, env.width), location_to_onehot(self.agent_location, env.height, env.width), location_to_onehot(env.target_location, env.height, env.width), self.carry_item))
    ...
    def get_shortest_path(self, env, max_steps=200):
        ...
        carry = 0
        while not done:
            state = self.prepare_state((location_to_onehot(item_pos, env.height, env.width), location_to_onehot(agent_pos, env.height, env.width), location_to_onehot(target_pos, env.height, env.width), carry))
...
def train(env: Environment, agent: Agent, episodes=1000, batch_size=1000, update_fre=1):
    """
    ...
    memory = ExperienceReplay(80000)  # Initialize experience replay memory
    ...
    for episode in range(episodes):
        ...
        while not (agent.agent_location[0] == env.target_location[0] and
                   agent.agent_location[1] == env.target_location[1] and agent.carry_item == 1 or count >= 400):

            # Agent chooses an action and updates its location and state
            action = agent.choose_action(env)
            old_state = agent.prepare_state((location_to_onehot(env.item_location, env.height, env.width), location_to_onehot(agent.agent_location, env.height, env.width), location_to_onehot(env.target_location, env.height, env.width), agent.carry_item)) + np.random.rand(1, statespace_size_g) / 20
            agent.move(action, env)
            new_state = agent.prepare_state((location_to_onehot(env.item_location, env.height, env.width), location_to_onehot(agent.agent_location, env.height, env.width), location_to_onehot(env.target_location, env.height, env.width), agent.carry_item)) + np.random.rand(1, statespace_size_g) / 20
...
def QL_test(env: Environment, agent: Agent, test_num=10000):
    ...
    for i in range(test_num):
        ...
        actual_length = manhattan_distance(env.agent_location, env.item_location) + \
                        manhattan_distance(env.item_location, env.target_location) + 1
