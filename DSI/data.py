...
# 数据整理器在深度学习中用于将多个数据点组合成一个批次，以便可以一次性将它们传递给模型进行训练或评估
@dataclass
class IndexingCollator(DataCollatorWithPadding):
    def __call__(self, features):# 这定义了当实例被当作函数调用时应该执行的操作。例如，如果你有一个名为collator的IndexingCollator实例，你可以像这样调用它：collator(features)。
